...
# 绘图函数
def plot_parallel(data, parallel_type, thread_or_proc_counts, matrix_sizes, serial_data):
    ...
    # plt.yscale('log')
    plt.tight_layout()
    plt.savefig(f'{parallel_type}_x.pdf')
    plt.show()


# 绘制串行和OpenMP的比较
plot_parallel(openmp_data, 'OpenMP', openmp_threads, matrix_sizes, serial_data)

# 绘制串行和Pthreads的比较
plot_parallel(pthread_data, 'Pthreads', pthread_threads, matrix_sizes, serial_data)

# 绘制串行和MPI的比较
plot_parallel(mpi_data, 'MPI', mpi_processes, matrix_sizes, serial_data)
